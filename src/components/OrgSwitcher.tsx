    ...
    <div className={className}>
      {atCap ? (
        <Link
          ...
          title={`You're managing ${cap?.current}/${cap?.limit} businesses. Upgrade for more capacity.`}
        >
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-amber-500" />
          {cap?.current}/{cap?.limit} · Upgrade
        </Link>
      ) : null}
      <OrganizationSwitcher
        // Keep Personal Account visible so existing solo users aren't disrupted.
        hidePersonal={false}
        ...
        afterSelectPersonalUrl="/assessments"
