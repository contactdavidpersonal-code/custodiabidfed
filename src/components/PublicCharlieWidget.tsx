...
const STORAGE_KEY = "custodia.publicCharlie.v1";

const GREETING: Msg = {
  role: "assistant",
  content:
    "Hi, I'm **Charlie** — Custodia's virtual compliance officer. Ask me anything about CMMC Level 1: who needs it, what the 15 safeguarding requirements actually mean, how to file a SPRS score, or whether your situation qualifies.\n\nWhat's going on?",
};

/**
 * Floating Charlie widget for public/marketing pages (blog, /cmmc-level-1).
 * Lives in the bottom-right; expands into a chat panel; backed by
 * /api/public-charlie. Rate-limit + abuse responses come back as a normal
 * reply plus a `cooldownSec` value — we lock the input and show a Claude-
 * style countdown until they can send again. Numeric limits are never shown.
 */
export function PublicCharlieWidget() {
  ...
  // Restore prior session so a visitor doesn't lose context on navigation.
  useEffect(() => {
    try {
  ...
  // Persist messages + active cooldown.
  useEffect(() => {
    try {
  ...
  async function send() {
    ...
    setSending(true);
    try {
      // The greeting bubble is UI-only; never send it to the model.
      const wire = next
        .filter(
          (m, i) =>
    ...
    <>
      ...
      {!open && (
        <button
          ...
          aria-label="Open Charlie — ask about CMMC Level 1"
          className="fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full bg-[#08201a] px-5 py-3 text-sm font-bold text-[#bdf2cf] shadow-2xl ring-1 ring-[#2f8f6d]/40 transition-all hover:bg-[#0c2a22] hover:shadow-[#2f8f6d]/30"
        >
          <span
            aria-hidden
            className="inline-block h-2 w-2 rounded-full bg-[#8dd2b1]"
          />
          Ask Charlie about CMMC L1
        </button>
      )}

      {/* Panel */}
      {open && (
        <div className="fixed bottom-5 right-5 z-50 flex h-[min(640px,calc(100vh-2.5rem))] w-[min(420px,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-xl border border-[#2f8f6d]/40 bg-white shadow-2xl">
          {/* Header */}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#8dd2b1]">
                  Virtual Compliance Officer · CMMC L1
                </div>
              </div>
            </div>
            <button
            ...
            {cooldownSec > 0 ? (
              ...
              >
                <textarea
                  ...
                  maxLength={1500}
                  disabled={sending}
                  placeholder="Ask about CMMC Level 1…"
...
/**
 * Tiny, safe Markdown renderer for Charlie replies. We escape HTML first,
 * then re-introduce a small whitelist: **bold**, *italic*, `code`, links
 * `[text](url)`, lists, and paragraphs. No images, no raw HTML, no script.
 * URLs are restricted to http(s) and same-origin paths starting with "/".
 */
