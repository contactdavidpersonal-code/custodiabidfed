  ...
  return (
    <main className="min-h-screen bg-[#f7f7f3] text-[#10231d]">
      ...
      <section className="mx-auto max-w-5xl px-4 py-6 md:px-6 md:py-8">
        ...
        {display.length === 0 ? (
          <div className="mt-6 border border-dashed border-[#cfe3d9] bg-white px-6 py-10 text-center md:px-10 md:py-14">
            ...
            <p className="mt-4 font-serif text-lg font-bold text-[#10231d]">
              No opportunities yet.
            </p>
            <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-[#456c5f]">
              {naics.length === 0
                ? "Add at least one NAICS code on your bid profile so the radar knows what to watch for."
                : "The radar runs every Monday morning. New SAM.gov notices matching your NAICS codes will appear here."}
            </p>
            <div className="mt-5 flex flex-col items-center gap-2 sm:flex-row sm:justify-center">
              <a
                href={naics.length === 0 ? "/profile/bid-ready" : "/assessments"}
                className="bg-[#0e2a23] px-5 py-2.5 text-sm font-bold tracking-tight text-[#bdf2cf] transition-colors hover:bg-[#10342a]"
              >
                {naics.length === 0 ? "Set up bid profile" : "Back to workspace"}
              </a>
            </div>
          </div>
        ) : (
