...
| A05 | Security Misconfiguration | **PASS** | HSTS preload, X-Content-Type-Options, X-Frame-Options DENY, Referrer-Policy, Permissions-Policy, Cross-Origin-Opener-Policy `same-origin-allow-popups` (required for Clerk's Cloudflare Turnstile bot protection — pure `same-origin` severs the postMessage channel and hangs signup with Turnstile error 300030), Cross-Origin-Resource-Policy `same-site`, and Content-Security-Policy (Report-Only) on every response (`next.config.ts`). CSP enumerates Clerk + Vercel Blob hosts; Anthropic is server-to-server only. CSP enforcement (drop the `-Report-Only`) is gated on two clean weeks of telemetry. |
